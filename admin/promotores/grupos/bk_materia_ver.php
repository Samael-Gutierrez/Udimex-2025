...
<link rel='stylesheet' href='estilo_grupo.css'>
</head>
<body>
...
<?php
	...
	$i=0;
	$al="Los alumnos que pertenecen a este grupo son:<table border='0' bgcolor='#aaaaaa'><tr bgcolor='#ffffff'><th>#</th><th>Apellido Paterno</th><th>Apellido Materno</th><th>Nombre</th><th></th></tr>";
	$datos=b_algr($_GET['id']);
	while($fila=mysqli_fetch_assoc($datos)){
		$i=$i+1;
		$al=$al."<tr bgcolor='#ffffff'><td>$i</td><td>".$fila['ap_pat']."</td><td>".$fila['ap_mat']."</td><td>".$fila['nombre']."</td><td>
		<a href='alumno_quita.php?id=".$fila['id_alumno']."&gr=".$_GET['id']."'>

		<img src='../../general/imagen/cierra.png' width='15px' align='right'></a></td></tr>";
	}
	echo $al."</table><hr>";

	$sg="Si requieres agregar algún alumno a este grupo puedes seleccionarlo a continuación:<table border='0' bgcolor='#aaaaaa'><tr bgcolor='#ffffff'><th>Alumno</th><th>Fecha de Ingreso</th><th>Inscripción</th><th>Colegiatura semanal</th></tr>";
	$datos=b_algr(0);
	$fact=date('Y-m-d');
	while($fila=mysqli_fetch_assoc($datos)){
				...
				<tr bgcolor='#ffffff'>
					...
					<td><input type='hidden' name='grupo' value='".$_GET['id']."'><input type='date' name='fi' value='$fact'></td>
