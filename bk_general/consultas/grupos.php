...
function e_grupo($gr,$edo){
	$consulta="update grupo set estado=$edo where id_grupo=$gr";
	completa($consulta);	
}
...
function act_gru2($id,$gr){
	...
	//completa($consulta);
	echo "Consulta repetida, verificar grupos.php";
}

//Actualiza la carrera de un alumno
function elimina_alumno_grupo($grupo){
	$consulta="update alumno set id_grupo=0 where id_grupo=$grupo";
	completa($consulta);
}
...
function grupo_alumnos($id_grupo){
    $consulta="select count(id_alumno) as r from alumno where id_grupo=$id_grupo";
    return completa($consulta);
